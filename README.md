...
> 所以后续在图片表格识别方向的功能不一定会继续增强，生长方向还是会走向本意所指。

目前只支持txt和pdf两种格式
pdf处理分支：目前是写死的，直接走图片式表格数据的处理逻辑，全部要做 OCR 以及后续的复杂处理（每页数据都要过llm做分块，还会有tool call），会消耗一定token。
txt：索引阶段只有在确定章节名格式的环节，可能会依赖llm确定regex；分块时完全不依赖llm。
